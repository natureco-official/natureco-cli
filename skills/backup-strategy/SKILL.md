---
name: backup-strategy
description: Design backup strategies with 3-2-1 rule, encryption, and recovery testing
category: Intelligence & General
---

# Backup Strategy

## Overview
Design backup strategies with 3-2-1 rule, encryption, and recovery testing. This skill helps you apply structured, repeatable methods for consistent results.

## When to Use
- When you need to apply backup strategy best practices
- When establishing processes or standards for your workflow
- When training team members on backup strategy
- When automating or optimizing backup strategy tasks

## Instructions
1. **Assess**: Evaluate the current state, requirements, and constraints
2. **Plan**: Define the approach, steps, and success criteria
3. **Execute**: Follow the established patterns and best practices
4. **Verify**: Check results against expected outcomes and quality standards
5. **Iterate**: Refine based on feedback and lessons learned

## Examples
```
User: Help me apply backup strategy for my current project
Assistant: I'll help you apply backup strategy step by step...
```

## Related Skills
- Use with `workflow` tool for orchestrated execution
- Combine with `task` tool for delegated processing
- Reference `system-prompt` for system-level integration
